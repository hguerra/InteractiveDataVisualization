...
/**
 * Collection of algorithms found on various places on the web. Credit is given
...
public class ColorMath {
	...
	public static Color getComplement(Color input) {
		...
		logger.fine(String.format("Complement r=%d g=%s b=%s", rgb[0], rgb[1], rgb[2]));
		try {
	...
	/**
	 * Convert RGB to HSL components H ranges from 0-360, S and L ranges from 0
	 * to 100
	 * 
	 * @param r
	 *            Red
	 ...
	 *            Blue
	 * @param hsl
	 *            This array will be assigned the HSL values (3 ints)
	 */
	public static void rgb2hsl(int r, int g, int b, int hsl[]) {
		...
		float green = (g / 255f);
		float blue = (b / 255f);
		logger.fine(String.format("Converting r=%1.3f, g=%1.3f, b=%1.3f to HSL", red, green, blue));
		...
		logger.fine(String.format("Result of rgb2hsl: h=%d, s=%d, l=%d", hsl[0], hsl[1], hsl[2]));
	}
	 ...
	 * @param x
	 * @param rgb
	 *            This array will be assigned the RGB values (3 ints)
	 */
	public static void hsl2rgb(int h, int s, int l, int rgb[]) {
		...
		logger.fine(String.format("Converting h=%1.3f, s=%1.3f, l=%1.3f to RGB", hue, saturation, lightness));
		...
		rgb[1] = Math.round(g);
		rgb[2] = Math.round(b);
		logger.fine(String.format("Result of hsl2rgb: r=%d, g=%d, b=%d", rgb[0], rgb[1], rgb[2]));
	 ...
	 * 
	 * @see <a href="http://www.easyrgb.com/index.php?X=MATH&H=19#text19">
	 *      EasyRGB</a>
	 *
	 * @param v1
	 *            ?
	...
	/**
	 * Returns the relative luminosity (luminance) of the given color. The value
	 * ranges from 0-255. The weights were taken from
