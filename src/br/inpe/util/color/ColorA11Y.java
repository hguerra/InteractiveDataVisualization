...
public class ColorA11Y {
	...
	public static enum Method {
		W3C, Luminosity, LuminosityContrast
	}
	 ...
	 *            The second color
	 * @param method
	 *            The method used to calculate the difference score
	...
	public static boolean isGoodColorMix(Color color_a, Color color_b, Method method) {
		...
		case LuminosityContrast:
			...
			lum_b = ColorMath.getLuminosity(color_b);
			lum_n = 255.0f;
			double delta = 116.0 * Math.pow((Math.abs(lum_a - lum_b) / lum_n), 1.0 / 3.0);
			return delta > 100.0;
	...
	public static Color getReadableComplement(Color color) {
		...
		while (!isGoodColorMix(color, complement, Method.LuminosityContrast) && textHSL[2] < 100 && textHSL[2] > 0) {
			...
			logger.finest(String.format("Adjusting HSL: h=%3d s=%3d l=%3d", textHSL[0], textHSL[1], textHSL[2]));
	...
	/**
	 * Color difference as defined <a
	 * href="http://www.w3.org/TR/AERT#color-contrast>by W3c</a>
	 *
	 * @param a
	 *            First color
	 * @param b
