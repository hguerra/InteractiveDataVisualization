...
import br.inpe.triangle.wwj.layer.ScenarioController;
import br.inpe.triangle.wwj.layer.Scenario;
...
public class ScenarioControllerImpl implements ScenarioController {
	...
	private Scenario appFrame;
	private WorldWindowGLCanvas canvas;

	public ScenarioControllerImpl(Scenario appFrame) {
		this.appFrame = appFrame;
		this.canvas = appFrame.getWwd();
		this.view = appFrame.getView();
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.inpe.triangle.app.ScenarioController#changeLayer()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.inpe.triangle.app.ScenarioController#yearForward()
	 */
	@Override
		...
		if (appFrame.getActiveLayer() == "a") {
			...
			if (appFrame.getLayerChanger() < 10) {
				...
				appFrame.getWwd().getModel().getLayers()
						.remove(appFrame.getVegLayers().get(appFrame.getLayerChanger()));

				appFrame.getWwd().getModel().getLayers()
						.remove(appFrame.getAnnotationLayers().get(appFrame.getLayerChanger()));

				// add new

				appFrame.setLayerChanger(appFrame.getLayerChanger() + 1);

				appFrame.getWwd().getModel().getLayers().add(appFrame.getVegLayers().get(appFrame.getLayerChanger()));

				appFrame.getWwd().getModel().getLayers()
						.add(appFrame.getAnnotationLayers().get(appFrame.getLayerChanger()));

				// refresh

				appFrame.refreshActiveLayers(appFrame.getVegLayers().get(appFrame.getLayerChanger()));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.inpe.triangle.app.ScenarioController#yearBackward()
	 */
	@Override
