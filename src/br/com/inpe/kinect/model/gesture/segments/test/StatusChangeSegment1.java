...
public class StatusChangeSegment1 extends SkeletonPoints implements IGestureSegment{

	public StatusChangeSegment1(SimpleOpenNI context) {
		super(context);
	}
	...
	public EGestureResult checkGesture(int userId) {
		...
		// Hands between shoulder and hip
			if(getY(userId, JointID.RIGHT_HAND)< getY(userId, JointID.CENTER_SHOULDER) && getY(userId, JointID.RIGHT_HAND) > getY(userId, JointID.CENTER_HIP) &&
				...
				// Hands very close
					if(getX(userId, JointID.RIGHT_HAND) - getX(userId, JointID.LEFT_HAND) < 0){
