...
public class FingerContours {
	...
	private void preCodeImage(int[] in) {
		...
		for (int x = 0; x < w - 1; x++) {
			for (int y = 0; y < h - 1; y++) {
				b0 = in[ixy(x + 0, y + 0)] < threshold ? 0x00001000
						: 0x00000000;
				b1 = in[ixy(x + 1, y + 0)] < threshold ? 0x00000100
						: 0x00000000;
				b2 = in[ixy(x + 1, y + 1)] < threshold ? 0x00000010
						: 0x00000000;
				b3 = in[ixy(x + 0, y + 1)] < threshold ? 0x00000001
						: 0x00000000;
	...
	private int findIsolines(int[] in) {
		...
		while (i < w * h) {
			...
			case CASE5: // CASE 5, saddle
				avg = 0.25 * (double) (in[ixy(x + 0, y + 0)]
						+ in[ixy(x + 1, y + 0)] + in[ixy(x + 0, y + 1)] + in[ixy(
						x + 1, y + 1)]);
				if (avg > threshold) {
					if (fromedge == 3) { // treat as case 1, then switch code to
											// case 4
						toedge = 2;
						cd[next] = CASE4;
					...
					if (fromedge == 3) { // treat as case 7, then switch code to
											// case 13
						toedge = 0;
						cd[next] = CASE13;
			...
			case CASE10: // CASE 10, saddle
				avg = 0.25 * (double) (in[ixy(x + 0, y + 0)]
						...
						x + 1, y + 1)]);
				if (avg > threshold) {
					if (fromedge == 0) { // treat as case 8, then switch code to
											// case 2
						toedge = 3;
						cd[next] = CASE2;
					...
					if (fromedge == 2) { // treat as case 14, then switch code
											// to case 11
						toedge = 3;
						cd[next] = CASE11;
	...
	public double measureAngle(int contour, int i) {
		...
		if (Double.isNaN(rads)) {
			applet.println("oops: " + befx + " " + befy + " " + befl + " "
					+ aftx + " " + afty + " " + aftl);
	...
	public boolean containsContour(int k1, int k2) {
		if (bbIntersect(k1, k2)) {
			...
			if (contains(k1, minx, miny) && contains(k1, maxx, miny)
	...
	public boolean containsBoundingBox(int k, double minx, double miny,
			double maxx, double maxy) {

		if (contains(k, minx, miny) && contains(k, maxx, miny)
	...
	public void drawContourDashed(int contour) {
		...
		for (int i = 0; i < getContourLength(contour); i++) {
			...
			int tx = (xi + xj) / 2;
			int ty = (yi + yj) / 2;
			int nx = (int) (10 * (getContourY(contour, i) - getContourY(
					contour, i - 1)));
			int ny = (int) (10 * (getContourX(contour, i - 1) - getContourX(
					contour, i)));
			...
			// g.drawLine((int)getContourX(contour, i),
			// (int)getContourY(contour, i), (int)getContourX(contour, i),
			// (int)getContourY(contour, i));
		}
	}
	...
	public PVector[] getContourPoints(int contour) {
		...
		for (int i = 0; i < getContourLength(contour); i++) {
			...
			int yi = (int) ((getContourY(contour, i) - cy) * s + cy);
			/*
			 * int xj = (int)((getContourX(contour, i - 1) - cx) * s + cx); int
			 * yj = (int)((getContourY(contour, i - 1) - cy) * s + cy); int tx =
			 * (xi + xj) / 2; int ty = (yi + yj) / 2; int nx = (int)(10 *
			 * (getContourY(contour, i) - getContourY(contour, i - 1))); int ny
			 * = (int)(10 * (getContourX(contour, i - 1) - getContourX(contour,
			 * i)));
			 */
