...
public class ShapefilesLayerChooserTest extends ApplicationTemplate {
	public static class AppFrame extends ApplicationTemplate.AppFrame {
		...
		public AppFrame() {
			...
			this.fc.addChoosableFileFilter(new FileNameExtensionFilter(
					"ESRI Shapefile", "shp"));
		}
		...
		protected void makeControlPanel() {
			...
			panel.setBorder(new CompoundBorder(BorderFactory.createEmptyBorder(
					0, 9, 9, 9), new TitledBorder("Shapefiles")));
			JPanel buttonPanel = new JPanel(new GridLayout(0, 1, 0, 5)); // nrows,
																			// ncols,
																			// hgap,
																			// vgap
			...
			JPanel pickPanel = new JPanel(new GridLayout(1, 1, 10, 10)); // nrows,
																			// ncols,
																			// hgap,
																			// vgap
			pickPanel
					.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10)); // top,
																					// left,
																					// bottom,
																					// right
			...
			this.pickCheck.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent actionEvent) {
					enablePicking(((JCheckBox) actionEvent.getSource())
							.isSelected());
				}
			});
			...
			this.dragCheck.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent actionEvent) {
					enableDragging(((JCheckBox) actionEvent.getSource())
							.isSelected());
				}
			});
		...
		public void showOpenURLDialog() {
			String retVal = JOptionPane.showInputDialog(this,
					"Enter Shapefile URL", "Open",
					JOptionPane.INFORMATION_MESSAGE);
			if (WWUtil.isEmpty(retVal)) // User cancelled the operation entered
										// an empty URL.
				return;

			URL url = WWIO.makeURL(retVal);
			if (url == null) {
				JOptionPane.showMessageDialog(this, retVal
						+ " is not a valid URL.", "Open",
		...
		public void run() {
			try {
				...
				for (int i = 0; i < layers.size(); i++) {
					...
					layers.get(i).setPickEnabled(
							this.appFrame.pickCheck.isSelected());
				}
		...
		protected List<Layer> makeShapefileLayers() {
			if (OpenStreetMapShapefileLoader.isOSMPlacesSource(this.source)) {
				Layer layer = OpenStreetMapShapefileLoader
						.makeLayerFromOSMPlacesSource(source);
