...
public class AdalineTraining {
	public static void main(String[] args) {
		...
		for (int i = 0; i < pesos.length; i++) {
			System.out
					.println(String.format("Peso final %s = %s", i, pesos[i]));
	...
	public double[] executarFaseTreinamento(double[][] amostras, double[] saidas) {
		...
		// 6. Repetir as instrucoes equanto o erro estiver fora da precisao
		// requerida.
		...
		do {
			...
			for (int k = 0; k < amostras.length; k++) {
				...
				// Pode ser usado um vetor para guardar os resultados e depois
				// realizar a soma.
				// Contudo, achei mais facil ja fazer a soma diretamente.
				double u = 0.0;

				// A inversao das variaveis i e k corresponde a transposta da
				// matriz.
				for (int i = 0; i < pesos.length; i++) {
					u += pesos[i] * amostras[i][k];
				}

				// 6.2.2. É realizado o ajuste de pesos e marcamos que existe
				// erro.
				for (int i = 0; i < pesos.length; i++) {
					pesos[i] = pesos[i] + taxaAprendizado * (saidas[k] - u)
							* amostras[i][k];
				}
			}
